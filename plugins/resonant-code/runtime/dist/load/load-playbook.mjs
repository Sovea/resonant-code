 ...
 */
export function discoverBuiltinLayers(builtinRoot        )                      {
 ...
 */
export function resolveExtendedLayers(extendsEntries          , layers                     )           {
  const selected           = [];
 ...
 */
export function loadDirectiveFile(filePath        , layerId        )              {
 ...
 */
export function loadLocalPlaybook(filePath         )                       {
  ...
  return {
    ...
    additions: Array.isArray(parsed.additions)
      ? parsed.additions.map((item) => normalizeDirective(item                           , 'local', filePath, 'local-addition'))
      : [],
  };
}
  ...
  return input.map((example) => {
    ...
    return {
      avoid: item.avoid && typeof item.avoid === 'object'
        ? { code: String((item.avoid                           ).code ?? '') }
        : undefined,
      good: item.good && typeof item.good === 'object'
        ? { code: String((item.good                           ).code ?? '') }
        : undefined,
