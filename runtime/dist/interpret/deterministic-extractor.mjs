import { COMPATIBILITY_REQUIREMENTS, INTERFACE_SENSITIVITIES, MIGRATION_PHASES, OPERATIONS, OPTIMIZATION_TARGETS, PROJECT_STAGES, REFACTOR_TOLERANCES, REVIEW_GOALS, RISK_LEVELS, SCOPE_SIZES, TASK_KINDS, hasEnumValue } from "../intent/schema.mjs";
	...
	interpret(task) {
		...
		return {
			...
			context: {
				...
				risk_level: toField(context.risk_level, explicitRiskLevel ? "explicit" : "deterministic", explicitRiskLevel ? 1 : .65, explicitRiskLevel ? "provided directly via task input" : "derived from project stage, constraints, optimization target, and task text"),
				...
				compatibility_requirement: toField(context.compatibility_requirement, explicitCompatibilityRequirement ? "explicit" : "deterministic", explicitCompatibilityRequirement ? 1 : context.compatibility_requirement === "none" ? .5 : .75, explicitCompatibilityRequirement ? "provided directly via task input" : "derived from compatibility and migration language in task constraints"),
				interface_sensitivity: toField(context.interface_sensitivity, explicitInterfaceSensitivity ? "explicit" : "deterministic", explicitInterfaceSensitivity ? 1 : context.interface_sensitivity === "unknown" ? .35 : .7, explicitInterfaceSensitivity ? "provided directly via task input" : "derived from target paths and tags"),
				refactor_tolerance: toField(context.refactor_tolerance, explicitRefactorTolerance ? "explicit" : "deterministic", explicitRefactorTolerance ? 1 : .65, explicitRefactorTolerance ? "provided directly via task input" : "derived from operation and narrow-change constraints"),
				migration_phase: toField(context.migration_phase, explicitMigrationPhase ? "explicit" : "deterministic", explicitMigrationPhase ? 1 : context.migration_phase === "none" ? .45 : .7, explicitMigrationPhase ? "provided directly via task input" : "derived from migration phase language in the task"),
				review_goal: toField(context.review_goal, explicitReviewGoal ? "explicit" : "deterministic", explicitReviewGoal ? 1 : .65, explicitReviewGoal ? "provided directly via task input" : "derived from operation, optimization target, and task risk terms")
			},
