...
function semanticMerge(directives, observations, intent, contextProfile) {
	...
	const directiveModes = directives.map((directive) => {
		const linkedObservations = scopedObservations.filter((observation) => observation.verification.disposition !== "demote-to-ambient").filter((observation) => lexicalSimilarity(directive.description, observation.pattern) >= .2);
		const defaultDecision = deriveDirectiveDecision(directive, linkedObservations);
		const decision = applyContextAdjustments(directive, linkedObservations, defaultDecision, contextProfile, contextInfluences);
		return {
	...
	const contextTensions = scopedObservations.flatMap((observation) => buildTensions(observation, directives, contextProfile));
	return {
...
function applyContextAdjustments(directive, observations, defaultDecision, contextProfile, contextInfluences) {
	let decision = {
		...defaultDecision,
		context_applied: [...defaultDecision.context_applied]
	};
	if (contextProfile.optimization_target === "safety" && directive.prescription === "should" && defaultDecision.execution_mode === "ambient" && observations.length > 0 && isSafetyRelevantDirective(directive)) {
		decision = {
	...
	if (hasConstraint(contextProfile.hard_constraints, [
		...
		"preserve public api"
	]) && directive.prescription === "must" && decision.execution_mode === "enforce" && observations.some((observation) => [
		"legacy",
		"constraint",
		"migration"
...
*/
function buildTensions(observation, directives, contextProfile) {
	if (observation.verification.disposition === "demote-to-ambient") return [];
	if (observation.adherence_quality === "good") return [];
	const match = directives.map((directive) => ({
		directive,
		score: lexicalSimilarity(directive.description, observation.pattern)
	})).filter((item) => item.score >= .2).sort((a, b) => b.score - a.score)[0]?.directive;
...
*/
function deriveDirectiveDecision(directive, observations) {
	...
	if ((observations.map((observation) => lexicalSimilarity(directive.description, observation.pattern)).sort((a, b) => b - a)[0] ?? 0) < .2) return {
