...
/**
 * Parses the YAML subset used by playbook, local augment, RCCL, and lockfile files.
 */
export function parseYaml(text: string): YamlValue {
  ...
  function readQuotedScalar(startValue: string, startIndex: number, parentIndent: number): { value: string; nextIndex: number } {
    ...
    return { value: stripQuotes(parts.join(' ')), nextIndex: index };
  }

  function keyColonIndex(value: string): number {
    let quote: '"' | "'" | null = null;
    let escaped = false;
  ...
  function readBlockScalar(startIndex: number, parentIndent: number, style: '|' | '>'): { value: string; nextIndex: number } {
    ...
    while (index < lines.length) {
      ...
      if (!trimmed) {
        ...
        content.push(raw.slice(Math.max(blockIndent, 0)));
      }
      index += 1;
    }
    const value = style === '>'
...
/**
 * Serializes lockfile and runtime output objects to deterministic YAML.
 */
export function toYaml(value: YamlValue, indent = 0): string {
  ...
  if (value && typeof value === 'object') {
    return Object.keys(value).map((key) => {
      ...
      return `${spaces}${emitScalar(key)}: ${emitScalar(child as Scalar)}\n`;
    }).join('');
  }
