...
export function resolveExecutionModes(
  ...
  const contextTensions = relations
    .filter((relation) => relation.relation === 'tension')
    .flatMap((relation) => {
      ...
      const record: SemanticMergeTensionRecord = {
        directive_id: directive.id,
  ...
  for (const relation of relations.filter((item) => item.relation === 'anti-pattern-suppress')) {
    reviewFocus.push({
      kind: 'anti-pattern',
...
function deriveDirectiveDecision(
  ...
  const hasSuppress = relations.some((relation) => relation.relation === 'anti-pattern-suppress');
  if (hasSuppress) {
    return {
      execution_mode: 'suppress',
...
function applyContextAdjustments(
  ...
  if (
    ...
    && directive.prescription === 'should'
    && isBroadScopeDirective(directive)
  ) {
    decision = {
      ...decision,
...
function isCompatibilitySensitiveDirective(directive: Directive): boolean {
  return directive.type === 'constraint' || directive.rccl_immune === true || directive.prescription === 'must';
}

function isBroadScopeDirective(directive: Directive): boolean {
