...
import type { EvaluateInput, LockfileDirectiveEntry } from './types.ts';

/**
 * Updates the runtime lockfile with per-directive quality signals.
 */
export function evaluateGuidance(input: EvaluateInput): Record<string, LockfileDirectiveEntry> {
...
function loadLockfile(filePath: string): Record<string, LockfileDirectiveEntry> {
  if (!existsSync(filePath)) return {};
  return parseYaml(readFileSync(filePath, 'utf-8')) as Record<string, LockfileDirectiveEntry>;
}
