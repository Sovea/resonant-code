...
function buildSingleTaskCandidateSchema(): unknown {
  return {
    type: 'object',
...
function fieldSchema(enumValues?: readonly string[]): unknown {
  const value = enumValues ? { enum: Array.from(enumValues) } : { type: 'string' };
  return {
    type: 'object',
    additionalProperties: false,
    properties: {
...
function listFieldSchema(): unknown {
  return {
    type: 'object',
    additionalProperties: false,
    properties: {
...
function buildInterpretationRecommendation(ambiguityHints: string[], candidatePath: string) {
  const shouldUseHostCandidate = ambiguityHints.length > 0;
  return {
    shouldUseHostCandidate,
    reason: shouldUseHostCandidate
      ? `Host-agent candidate recommended because ${ambiguityHints.join('; ')}.`
      : 'Host-agent candidate is optional because the task already carries concrete operational signals.',
    nextStep: shouldUseHostCandidate
...
function buildClarificationHints(task: CompileTaskInput, ambiguityHints: string[]): string[] {
  const hints: string[] = [];
...
function buildInterpretationPrompt(task: CompileTaskInput): string {
  return [
...
function buildAmbiguityHints(task: CompileTaskInput): string[] {
  const hints: string[] = [];
